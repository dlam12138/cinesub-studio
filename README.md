...
三种模式都只使用 faster-whisper。默认旧调用仍只把置信度和异常检测用于日志、报告及“建议人工听取”标记，不会自动换模型。v0.7 的“识别质量闭环”可选启用词级时间戳、确定性重切分和固定配方局部重试；只有质量优先模式在预算、硬拒绝和事务校验通过时才会局部替换 suspicious cue，并写入 ASR 审计报告。
