...
如果视频带有硬字幕，可使用独立 CLI 把 OCR 双语字幕与 ASR/译文进行离线对照，生成差异报告和候选初筛。该流程默认不访问网络、不覆盖字幕，也不会把 OCR 当成人工金标。FunASR、WhisperX、ASR candidate、`mixed-route-v1` 和 segment routing 已退出产品链路；一次性可执行脚本、专属测试、内部验收记录和研究工作区均不进入正式源码与便携包，也不会参与正常任务。OCR 结果解释见 [`docs/ocr_weak_evidence_evaluation.md`](docs/ocr_weak_evidence_evaluation.md)。
...
以下内容记录 Electron 便携版引入前的历史启动方式，仅用于维护参考；当前 0.6.2 已使用 Electron 免安装壳。项目仍不提供安装器、代码签名、自动更新、模型中心或配音/TTS。
